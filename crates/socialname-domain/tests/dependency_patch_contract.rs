...
const DEPLOYMENT_DOCKERFILES: [&str; 3] = [
    include_str!("../../../deploy/server/Dockerfile"),
    include_str!("../../../deploy/canary/Dockerfile"),
    include_str!("../../../deploy/worker/Dockerfile"),
];
...
fn glib_018_soundness_patch_is_pinned_and_auditable() {
    ...
    assert!(
        DEPLOYMENT_DOCKERFILES
            .iter()
            .all(|dockerfile| dockerfile.contains("COPY vendor ./vendor"))
    );
