...
explicit workspace/API-key, plan-reconciliation, signed rule-pack, and
verified target-deletion operator commands, exposes authenticated private
workspace/search/watch,
...
The serving binary reads seven environment variables:
...
compares all 38 published operation scopes with the scopes used to construct
the router and requires every published method/path to resolve behind
authentication. JSON Schema drift, OpenAPI/SSE linkage, and generated bytes
are checked by `socialname-protocol`; see
...
`GET /v1/workspace/plan` also requires `workspace:read`. It returns the closed
provider-neutral plan, derived database-time state/capabilities, revision, and
timestamps. New managed-search/watch admission and due-watch scheduling fail
closed when the capability is absent, while existing reads, cancellation,
export, watch pause/delete, and exact idempotency replay remain available. The
schema-owner `reconcile-plan-entitlement` command hashes its external event
identity, advances one optimistic revision, and stores an append-only event
plus target-free audit record. See
[Plan entitlements and billing boundary](plan-entitlements-billing.md).

The Team routes reuse the workspace as one organization and require both their
published existing scope and the authenticated membership's active closed
role. Owners and administrators manage eligible memberships, retention, and
review assignment; an assigned non-viewer acknowledges and resolves a review.
The private provisioning subject never enters a resource or ordinary
application-role read, while audit output excludes details and targets. See
...
`POST /v1/searches/{search_id}/completion-webhook` requires `search:write` and
idempotently binds one existing active tenant-local webhook endpoint. The
matching `GET` requires `search:read`; `DELETE` requires `search:write` and
cancels the binding plus any queued, leased, or retry-scheduled delivery.
Completed and failed searches enqueue exactly once whether registration or
terminal state commits first. Cancelled searches never enqueue. The resource
and signed body exclude search targets/results and endpoint destinations. See
...
`GET /v1/developer/report` independently requires `usage:read` and accepts only
`24h`, `7d`, or `30d`. It returns current UTC tenant/key quota, admitted
search/target-pair usage, unfinished search backlog, and fixed success and
latency objective states from one database snapshot. It exposes no target,
site, search, consent, key, destination, or idempotency identifier. See
